...

{
 //crinado e iniciando váriavel para executar comando SQL no banco de dados
  $sql = $conecta->prepare("select * from Equipes where nomeEquipe like '".$buscaIdEquipe."%' or codEquipe = '".$buscaIdEquipe."'");
}  
   // executa a sql
   if($sql -> execute()){ // verifica se há sucesso ao executar sql
	if ($sql -> rowCount() > 0){ // verifica se há informações   
		...
		while ($campo = $sql -> fetch (PDO::FETCH_OBJ)){
			...
			echo "<td><font color='#0b2e59'> <input class='editarEquipe' type='text' name='novoNomeEquipe' value=' ". $campo -> nomeEquipe ."' </td>";
