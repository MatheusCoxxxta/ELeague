...
<head>
    ...
    <link rel="stylesheet" href="css/testandoTabela.css">
...
}else
{
  ...
  //conexão 
  //where QuantEquipe = '".$buscaEvento."' or  jogEquipe = '".$buscaEvento."' or anoEvento = '".$buscaEvento."'
	
  //mostra os produto (seleciona todos)
  $sql = $conecta->prepare("select * from Campeonato where QuantEquipe = '".$buscaEvento."' or  nomeCamp = '".$buscaEvento."' or anoEvento = '".$buscaEvento."' ");
}  
   // executar a sql
   if($sql -> execute()){ // sucesso ao executar sql?
	if ($sql -> rowCount() > 0){ // há informações?	   
		...
		while ($campo = $sql -> fetch (PDO::FETCH_OBJ)){// percorre consulta
			...
			echo "<td><b id='col1'><font color='#0b2e59'> <a href='Equipes.php?ano=".$campo -> anoEvento."'> ". $campo -> nomeCamp ." ". $campo -> anoEvento ." </td> </a>";
